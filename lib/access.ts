...
export async function learningMember(){const m=await member();if(!m)return null;const selected=(await listItems(m,'learning_path'))[0]?.data.role;return {...m,role:selected==='agent'||selected==='owner'?selected:m.role,pathSelected:selected==='agent'||selected==='owner'}}
export async function requireMember(){const m=await learningMember();if(!m)throw Error('UNAUTHENTICATED');if(!m.enrolled)throw Error('ENROLLMENT_REQUIRED');return m}
