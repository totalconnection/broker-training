...
export async function PATCH(r:Request){try{await checkOrigin();const m=await requireMember();const {role}=z.object({role:z.enum(["agent","owner"])}).strict().parse(await r.json());const current=(await listItems(m,"learning_path"))[0];await saveItem(m,"learning_path",{role},current?.id);if(!m.demo){const {database}=await import("@/lib/db");await database().query("UPDATE enrollments SET role=$1 WHERE user_id=$2",[role,m.id])}return NextResponse.json({ok:true})}catch(e){return error(e)}}
